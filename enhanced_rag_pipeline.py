...
# --- Main Execution Block for Enhanced HackRx API Simulation ---
if __name__ == "__main__":
    print("Starting the ENHANCED HackRx RAG pipeline simulation...")

    # Define the sample request based on HackRx API specification
    sample_hackrx_request = {
    ...
    # Redirect all console output to a log file for detailed inspection
    with open("enhanced_rag_pipeline_log.txt", "w", encoding="utf-8") as f:
        import sys
        original_stdout = sys.stdout
        sys.stdout = f

        try:
            hackrx_final_response = run_enhanced_hackrx_pipeline(
                questions=sample_hackrx_request["questions"],
                documents_url=sample_hackrx_request["documents"]
            )
        except Exception as e:
            print(f"FATAL ERROR during enhanced pipeline execution: {e}")
            traceback.print_exc()
            hackrx_final_response = {"answers": ["A fatal error occurred during pipeline execution."]}
        finally:
            sys.stdout = original_stdout

    print("\n--- ENHANCED RAG Pipeline Execution Complete ---")
    print("Detailed logs saved to 'enhanced_rag_pipeline_log.txt'.")
    print("\n--- Final Answer (HackRx API Format) ---")
    print(json.dumps(hackrx_final_response, indent=2))
    
    # Run validation
    validate_test_cases(hackrx_final_response)
